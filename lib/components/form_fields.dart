...
class LinksFormField extends StatelessWidget {
  const LinksFormField({super.key, required this.ctrl, required this.label, required this.vText});
  final TextEditingController ctrl;
  final String label;
  final String vText;
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      decoration: InputDecoration(
        prefixIcon: const Icon(Icons.create_rounded),
        labelText: label,
        helperText: '*optional (https://www.example.com)'
      ),
      controller: ctrl,
      keyboardType: TextInputType.text,
      validator: (value) {
